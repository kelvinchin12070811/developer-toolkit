...
export function CharactersGeneratorPage() {
  ...
  return (
    <GeneratorCard title="Characters Generator" valueToCopy={characters} onGenerate={generateCharacters}>
      <div>
      ...
      <div className="flex flex-col gap-0.5">
        ...
        <div className="flex gap-2 items-center">
          <input type="range" className="range w-full" min={1} max={255} step={1} value={targetLength}
                 onChange={e => setTargetLength(parseInt(e.target.value) || 1)} />
          <input className="input h-7 w-12" value={targetLength}
                 onChange={e => setTargetLength(parseInt(e.target.value) || 1)} />
        </div>
        ...
        <div className="flex flex-col gap-2 items-stretch">
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-1">
            <label className="label">
              <input type="checkbox" className="toggle" checked={shouldGenerateLowercase}
                     onChange={e => setShouldGenerateLowercase(e.target.checked)} />
              Lowercase
            </label>
            <label className="label">
              <input type="checkbox" className="toggle" checked={shouldGenerateUppercase}
                     onChange={e => setShouldGenerateUppercase(e.target.checked)} />
              Uppercase
            </label>
            <label className="label">
              <input type="checkbox" className="toggle" checked={shouldGenerateDigits}
                     onChange={e => setShouldGenerateDigits(e.target.checked)} />
              Digits
            </label>
            <label className="label">
              <input type="checkbox" className="toggle" checked={shouldGenerateSymbols}
